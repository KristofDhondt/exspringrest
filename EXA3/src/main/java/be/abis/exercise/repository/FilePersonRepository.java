...
public class FilePersonRepository implements PersonRepository {
		...
		try {
			...
			while ((s = br.readLine()) != null) {
				...
				if (!vals[0].equals("")) {
					...
					LocalDate birthdate = LocalDate.parse(vals[3], formatter);
					...
					p.setBirthdate(!birthdate.equals("null") ? birthdate : null);
	...
	private StringBuilder parsePerson(Person p) {
		...
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
		String birthdayString = formatter.format(birthdate);
		
		sb.append(p.getPersonId() + ";").append(p.getFirstName() + ";").append(p.getLastName() + ";")
				.append(birthdayString + ";").append(p.getEmailAddress() + ";")
